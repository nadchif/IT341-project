...
class frontSideUser {
	...
	protected static $tbl_name = "back_end_users";
	/*-Comon SQL Queries-*/
	function db_fields(){
		global $mydb;
	...
	function allUsers(){
		global $mydb;
		$mydb->setQuery("SELECT * FROM back_end_users");
	...
	static function Authenticate($uname="", $h_upass=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `back_end_users` WHERE `uname`='". $uname ."' and `u_pass`='". $h_upass ."'");
		$rows = $mydb->loadSingleResult();
			$_SESSION['user_id'] 	= $rows->user_id;
			$_SESSION['users_name']  = $rows->users_name;
			$_SESSION['usname'] = $rows->uname;
